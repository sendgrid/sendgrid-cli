...
const subusersProto = {
  ...
  getTeammatesForSubuser: function (subuser) {
    ...
    if (process.env.SENDGRID_TOKEN) {
      // Override Authorization header with token method
      debug('Using SENDGRID_TOKEN');
      options.headers = {
        'Authorization': 'Token ' + process.env.SENDGRID_TOKEN,
        'On-Behalf-Of': subuser
      };
    } else {
      throw new Error('getTeammatesForSubuser requires SENDGRID_TOKEN to be set!');
    }
    options.method = 'GET';
